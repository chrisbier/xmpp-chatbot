...
class QueryBot(slixmpp.ClientXMPP):
    ...
    async def message(self, msg):
        ...
        """
        data = {"words": list(), "reply": list(), "queue": list()}
        ...
        for job in data["queue"]:
            ...
            data["reply"].append(
                self.functions[keyword].format(
                    query=query, target=target, opt_arg=opt_arg
                )
            )
    ...
    def build_queue(self, data, msg):
        ...
        for x in enumerate(data["words"]):
            ...
            if keyword.startswith("!") and keyword in StaticAnswers().keys(
                "no_arg_keywords"
            ):
                data["queue"].append({keyword: [None, None]})

            # matching all words starting with ! and member of keywords
            elif keyword.startswith("!") and keyword in StaticAnswers().keys(
                "keywords"
            ):
                ...
                if misc.validate(keyword, target):
                    logging.debug(
                        "Item added to queue %s" % {str(keyword): [target, opt_arg]}
                    )
